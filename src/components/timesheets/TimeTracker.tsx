...
export function TimeTracker({ projects, tasks, onEntryCreated }: TimeTrackerProps) {
  ...
  const { radius, circumference, progress } = progressRing();
          ...
          <div className="relative flex-shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] text-white/30 uppercase tracking-wider">
                {state === "running" ? "Tracking" : state === "paused" ? "Paused" : "Ready"}
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 w-full space-y-4">
            <div className="flex items-center gap-3">
              <Input
                ...
                className="bg-[#0b0d10] border-white/10 text-white flex-1"
              />
              <button
                onClick={() => setBillable(!billable)}
                className={`px-3 py-2 rounded-md text-xs font-medium transition-colors ${
                  billable
                    ? "bg-[#8dc572]/20 text-[#8dc572] border border-[#8dc572]/30"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <Select value={taskId} onValueChange={setTaskId} disabled={!projectId || filteredTasks.length === 0}>
                <SelectTrigger className="bg-[#0b0d10] border-white/10 text-white">
                  <CheckCircle2 className="w-4 h-4 mr-2 text-white/30" />
            ...
            <div className="flex items-center gap-3">
              {state === "idle" && (
                <Button onClick={start} className="bg-[#6452db] text-white hover:bg-[#6452db]/90 flex-1">
                  <Play className="w-4 h-4 mr-2" /> Start Timer
                </Button>
              )}
              {state === "running" && (
                <>
                  <Button onClick={pause} variant="outline" className="border-white/10 text-white hover:bg-white/5 flex-1">
                    <Pause className="w-4 h-4 mr-2" /> Pause
                  </Button>
                  <Button onClick={stopAndSave} disabled={saving} className="bg-[#8dc572] text-black hover:bg-[#8dc572]/90 flex-1">
                    <Square className="w-4 h-4 mr-2" /> {saving ? "Saving..." : "Stop & Log"}
                  </Button>
                </>
              )}
              {state === "paused" && (
                <>
                  ...
                  <Button onClick={reset} variant="ghost" size="icon" className="text-white/30 hover:text-[#be6464]">
